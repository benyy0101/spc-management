...
| 기능               | 화면 / 엔드포인트       | 설명                                   | 상태                | 시각 자료          |
| ------------------ | ----------------------- | -------------------------------------- | ------------------- | ------------------ |
| 재무상태표 조회    | 웹 `Balance Sheet`      | 기준일 자산/부채/자본 잔액을 조회한다. | 구현 진행 범위 포함 | 스크린샷 추가 예정 |
| 손익계산서 조회    | 웹 `Profit & Loss`      | 기간 기준 수익/비용 흐름을 조회한다.   | 구현 진행 범위 포함 | 스크린샷 추가 예정 |
| 현금흐름표 조회    | 웹 `Cash Flow`          | 간접법 기준 현금흐름표를 조회한다.     | 구현 진행 범위 포함 | 스크린샷 추가 예정 |
| 재무제표 매핑 관리 | 웹 `Statement Mappings` | 계정과 재무제표 라인 매핑을 관리한다.  | 구현 진행 범위 포함 | 스크린샷 추가 예정 |

### 3.4 운영 통제 및 후속 기능

| 기능                              | 화면 / 엔드포인트                      | 설명                                                | 상태      | 시각 자료                 |
| --------------------------------- | -------------------------------------- | --------------------------------------------------- | --------- | ------------------------- |
| 마감 관리                         | 웹 `Close Periods`                     | 회계 기간 마감 상태를 조회/관리한다.                | 확장 진행 | 스크린샷 추가 예정        |
| 투자자 배분                       | 웹 `Allocations`, `Investor Positions` | pro-rata 기준 배분 결과와 투자자 포지션을 확인한다. | 확장 진행 | 스크린샷 / 영상 추가 예정 |
| 감사 로그 조회                    | 웹 `Audit Logs`                        | 변경 및 처리 이력을 확인한다.                       | 확장 진행 | 스크린샷 추가 예정        |
| 수기분개 / 승인 / 역분개 / 재처리 | API 및 운영 화면 확장                  | 내부통제와 운영 보완 기능이다.                      | 계획 포함 | 영상 추가 예정            |
